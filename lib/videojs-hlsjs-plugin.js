var Hlsjs = require('metacdn.hls.js');
        ...
        function _initHlsjs() {
            ...
            if (hlsjsConfig.event) {
                for (var x in hlsjsConfig.event) {
                    _hls.on(Hlsjs.Events[x], hlsjsConfig.event[x]);
                }
            }

            _hls.on(Hlsjs.Events.ERROR, function (event, data) { _onError(event, data, tech, _errorCounts); tech.trigger('error', event, data);});
            ...
            _hls.on(Hlsjs.Events.LEVEL_LOADED, function (event, data) {
                var isDVR = hlsjsConfig.isDVR != null ? hlsjsConfig.isDVR : false
                _duration = data.details.live && !isDVR ? Infinity : data.details.totalduration;
                tech.trigger('levelLoaded', event, data);
            });

            _hls.on(Hlsjs.Events.LEVEL_SWITCH, _onLevelSwitch);
            _hls.on(Hlsjs.Events.LEVEL_LOADING, _onLevelLoading);

            _hls.on(Hlsjs.Events.FRAG_LOADED, _onFragLoaded);
            _hls.on(Hlsjs.Events.LEVEL_SWITCHED, _onLevelSwitched);
            _hls.on(Hlsjs.Events.DESTROYING, _onDestroying);
